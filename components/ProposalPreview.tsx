  ...
  return (
    <div className="print-sheet">
      ...
      <div className="mb-6 grid grid-cols-[1fr_360px] gap-6">
        <section className="rounded-2xl border border-slate-200 p-4">
          ...
          <div className="space-y-2 text-sm">
            <div>단위사업명: {proposal.project_name || "-"}</div>
            <div>사업기간: {proposal.project_period || "-"}</div>
            <div>작성일: {proposal.submission_date || "-"}</div>
            <div>재원구분: {fundLabels[proposal.fund_type]}</div>
            <div>
              소요금액: 금 {formatCurrency(proposal.total_amount)}원정도
              {" ("}
              {numberToKorean(proposal.total_amount)}
              원정도)
            </div>
          </div>
        </section>
      ...
      <section className="mb-6">
        <div className="mb-3 text-sm font-semibold">내역</div>
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-slate-50">
              {["연번", "비목", "적요", "추정금액", "산출근거", "비고"].map((label) => (
      ...
      <section className="mb-8 rounded-2xl border border-slate-200 p-4 text-sm">
        <div className="mb-2 font-semibold">관련계획서</div>
